//$Id: NodeTable.java,v 1.6 2008/08/01 09:41:47 barathv Exp $
...
public class NodeTable implements JmxTableModelListener{
       // User code starts here
Vector moInDB;
String className = "Node"; //No I18N
    String[] names = {"MoNameIndex","IsRouter","InheritingOid4","InheritingTableName4"};//No I18N
    Object[] values = null;
    Hashtable hash = new Hashtable();
     private CompositeData getCompositeData(Node  mo,String moName) throws Exception
    {
	...
	values[2]=oid;
	values[3]=agentName.getChildTableName(oid);
	  try
	{
	...
	private NodeEntry entry = null;
	private String instrClassName = "com.adventnet.nms.jmxagent.NodeEntry";//No I18N
	...
	/**
	 * Initialises the NodeTable
	 */
	public NodeTable()
	{
		table = new Hashtable();
	}
	public NodeTable(AdventNet_WebNMS_MIB_JMX agentRef)
	{
		agentName = agentRef;
		table = new Hashtable();
                // User code starts here
	 ...
	 * @return TabularData containing the rows corresponding the NodeTable
	 */
	public TabularData getNodeTable()
	{
		try{
			if(table != null)
	 ...
	 */
	public void setNodeTable(TabularData data) throws Exception
	{
		...
		for(Enumeration e = data.enumerate();e.hasMoreElements();)
		{
			...
			if(table != null)
				entry = (NodeEntry)Utilities.getEntryFromCompositeData(table, comp, indexNames, instrClassName);
			else if(vec != null)
				entry = (NodeEntry)Utilities.getEntryFromCompositeData(vec, comp, indexNames, instrClassName);
    ...
    void initPSID()
    {
        psSelectMoID1 = agentName.rlAPI.getPreparedStatementID("SELECT MIN(ManagedObject."+RelationalUtil.getAlias("name") +") FROM ManagedObject , Node WHERE ManagedObject."+RelationalUtil.getAlias("moid")+ "= Node."+RelationalUtil.getAlias("moid"));//No I18N

        psSelectMoID2=agentName.rlAPI.getPreparedStatementID("SELECT MIN(ManagedObject."+RelationalUtil.getAlias("name") +") FROM ManagedObject , Node WHERE ManagedObject."+RelationalUtil.getAlias("moid")+ "= Node."+RelationalUtil.getAlias("moid")+ " AND ManagedObject."+RelationalUtil.getAlias("name")+ " >?"); //No I18N

        psSelectCount = agentName.rlAPI.getPreparedStatementID("SELECT COUNT(" + RelationalUtil.getAlias("moid") + ") FROM Node");//No I18N
    }
    ...
    public CompositeData getNode(String[] name)
    {
        Node node=null;
        try
        {
            node=(Node)agentName.topoAPI.checkOut(name[0],0,false,true);
            if(node!=null)
            {
