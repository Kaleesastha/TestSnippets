 //$Id: NetworkTable.java,v 1.6 2008/08/01 09:41:26 barathv Exp $
...
public class NetworkTable implements JmxTableModelListener{

     // User code starts here
Vector moInDB;
String className = "Network";//No I18N
    String[] names = {"MoNameIndex","Discover","DiscoverStatus","InheritingOid3","InheritingTableName3"};//No I18N
    Object[] values = null;
	...
	private String instrClassName = "com.adventnet.nms.jmxagent.NetworkEntry"; //No I18N
	...
	/**
	 * Initialises the NetworkTable
	 */
	public NetworkTable()
	{
		table = new Hashtable();
	}
	public NetworkTable(AdventNet_WebNMS_MIB_JMX agentRef)
	{
		agentName = agentRef;
		table = new Hashtable();
                // User code starts here
	 ...
	 */
	public void setNetworkTable(TabularData data) throws Exception
	{
		...
		for(Enumeration e = data.enumerate();e.hasMoreElements();)
		{
			...
			if(table != null)
				entry = (NetworkEntry)Utilities.getEntryFromCompositeData(table, comp, indexNames, instrClassName);
			else if(vec != null)
				entry = (NetworkEntry)Utilities.getEntryFromCompositeData(vec, comp, indexNames, instrClassName);
    ...
    void initPSID()
    {
        psSelectCount = agentName.rlAPI.getPreparedStatementID("SELECT COUNT(" + RelationalUtil.getAlias("moid") + ") FROM Network"); //No I18N

        psSelectMoID1 = agentName.rlAPI.getPreparedStatementID("SELECT MIN(ManagedObject."+RelationalUtil.getAlias("name") +") FROM ManagedObject , Network WHERE ManagedObject."+RelationalUtil.getAlias("moid")+ "= Network."+RelationalUtil.getAlias("moid"));//No I18N

        psSelectMoID2=agentName.rlAPI.getPreparedStatementID("SELECT MIN(ManagedObject."+RelationalUtil.getAlias("name") +") FROM ManagedObject , Network WHERE ManagedObject."+RelationalUtil.getAlias("moid")+ "= Network."+RelationalUtil.getAlias("moid")+ " AND ManagedObject."+RelationalUtil.getAlias("name")+ " >?");//No I18N

    }

    CompositeData getFirstMo(String[] name)
    {
    ...
    public CompositeData getNetwork(String[] name)
    {
        Network net=null;
        try
        {
            net=(Network)agentName.topoAPI.checkOut(name[0],0,false,true);
            if(net!=null)
            {

                return getCompData(net,name[0]);
            }
            return null;             
        }
        catch(Exception e)
