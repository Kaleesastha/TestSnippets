//$Id: CollectedGraphViewer.java,v 1.2.14.1 2012/04/05 08:22:35 wesley Exp $
...
public class CollectedGraphViewer extends JFrame  implements  Runnable,com.adventnet.apiutils.ParameterChangeListener
{
	...
	CollectedGraphController graphController = null;
...
//<UserCode_End_Bean_closeButton>
		...
		plotButton.setPreferredSize(new Dimension(plotButton.getPreferredSize().width+7,plotButton.getPreferredSize().height+3));
...
mainPanel.add(rangeSettingsPanel,BorderLayout.EAST);
...
setConstraints(0,4,1,1,0.0,0.0,cons.EAST,cons.NONE,inset,0,0);
...
//<End_setUpGUI_Container>
  ...
  public CollectedGraphViewer()
  {
	applet = NmsClientUtil.applet;
  init();
/*
     //<Begin_CollectedGraphViewer>
    pack();

    //<End_CollectedGraphViewer>
  ...
  */
  ...
  public CollectedGraphViewer(java.applet.Applet applet)
  {
    //<Begin_CollectedGraphViewer_java.applet.Applet>
    this.applet = applet;
    pack();
    this.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);

    //<End_CollectedGraphViewer_java.applet.Applet>
  }
  ...
  //<End_parameterChanged_com.adventnet.apiutils.ParameterObject>
    ...
    public void run()
    {
        ...
        graphController.fetchData(null,startTime,endTime);
        NmsClientUtil.normalCursor(this);
    }
    ...
    public void setController(CollectedGraphController cgc)
    {
    ...
    public void disableViewer(String message)
    {
      ...
      public void disableViewer(String message,boolean flag)
      {
	      ...
	      clearGraphButton.setEnabled(flag);
	      lineButton.setEnabled(flag);
	      barButton.setEnabled(flag);
	      areaButton.setEnabled(flag);
	      scatterButton.setEnabled(flag);
	      xystepButton.setEnabled(flag);
	      tableButton.setEnabled(flag);
	      
	      ((DefaultTableModel)tableView.getModel()).setDataVector(new Vector(), new Vector());
	      constructTable();
	...
	disableViewer(null,true);    
        ...
        try
        {
            ...
            for(Enumeration e=data.keys();e.hasMoreElements();)
            {
                ...
                if(type == 1)
                {
                	values = (Long[])cdata.getValues();
                }
                else if(type == 3)
                {
                	dvalues = (Double[])cdata.getValues();
                }
                if(firstTime)
                {
                ...
                for(int y=0;y<times.length;y++)
                {
                    ...
                    {
                    	yVals[x][y] = dvalues[y].doubleValue();
                    }
                }
                x++;
            }
    ...
    {
	    baseChart.clearChart();
	    disableViewer("No data available",false);//No I18N
    }
	 ...
	 DecimalFormat df = new DecimalFormat("0.####");
        if (data != null)
        {
            ...
            for (Enumeration e = data.keys(); e.hasMoreElements();)
            {
                ...
                {
                    doubleValues = (Double[])cdata.getValues();
                }
                ...
                for (int i = 0; i < times.length; i++)
                {
                    ...
                    {
                        rowdata.add(df.format(doubleValues[i]));
                    }
