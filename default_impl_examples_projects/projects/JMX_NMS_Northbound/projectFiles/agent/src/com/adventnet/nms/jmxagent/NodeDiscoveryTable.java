  //$Id: NodeDiscoveryTable.java,v 1.2 2007/04/30 17:39:31 tinku Exp $
 ...
 * nodeDiscoveryTable group
 */


public class NodeDiscoveryTable{
	...
	if(agentName.topoConfImpl.nodeDiscoveryTable != null)
	    table = agentName.topoConfImpl.nodeDiscoveryTable;
	// User code ends here
	}
	...
	private NodeDiscoveryEntry entry = null;
	private String instrClassName = "com.adventnet.nms.jmxagent.NodeDiscoveryEntry";//No I18N
	private String[] indexNames = new String[]{"NodeIP"};//No I18N
	...
	/**
	 * Initialises the NodeDiscoveryTable
	 */
	public NodeDiscoveryTable()
	{
		table = new Hashtable();
	}
	public NodeDiscoveryTable(AdventNet_WebNMS_MIB_JMX agentRef)
	{
		agentName = agentRef;
		table = new Hashtable();
	}
	 ...
	 * @return TabularData containing the rows corresponding the NodeDiscoveryTable
	 */
	public TabularData getNodeDiscoveryTable()
	{
	     // User code starts here
	 ...
	 */
	public void setNodeDiscoveryTable(TabularData data) throws Exception
	{
		...
		for(Enumeration e = data.enumerate();e.hasMoreElements();)
		{
			...
			if(table != null)
				entry = (NodeDiscoveryEntry)Utilities.getEntryFromCompositeData(table, comp, indexNames, instrClassName);
			else if(vec != null)
				entry = (NodeDiscoveryEntry)Utilities.getEntryFromCompositeData(vec, comp, indexNames, instrClassName);
			...
			else if(comp.getOperationType().equals(CompositeData.MODIFIED))
			{
			 ...
			 // User code starts here
	    if(!agentName.topoConfImpl.setSeedData(4))
