# Stored Procedure to execute alter query if a table exists
DELIMITER $$
...
alter table ManagedObject add column DISCRIMINATOR varchar(40) NOT NULL after `MOID`;
...
call Edit_table('WebNmsDB','CnpThirdPartyAppServer','alter table CnpThirdPartyAppServer drop primary key');
call Edit_table('WebNmsDB','CnpThirdPartyAppServer','alter table CnpThirdPartyAppServer drop column OWNERNAME');
call Edit_table('WebNmsDB','CnpThirdPartyAppServer','alter table CnpThirdPartyAppServer add column MOID bigint(20) first;');
call Edit_table('WebNmsDB','CnpThirdPartyAppServer','update CnpThirdPartyAppServer set moid = (select moid from ManagedObject where CnpThirdPartyAppServer.name = ManagedObject.name)');
call Edit_table('WebNmsDB','CnpThirdPartyAppServer','alter table CnpThirdPartyAppServer add primary key (MOID)');
call Edit_table('WebNmsDB','CnpThirdPartyAppServer','alter table CnpThirdPartyAppServer add constraint `FKF75D840E1798EF8` FOREIGN KEY (`MOID`) REFERENCES `ManagedObject` (`MOID`) ON DELETE CASCADE');
call Edit_table('WebNmsDB','CnpThirdPartyAppServer','alter table CnpThirdPartyAppServer drop column NAME');
# index creation for CnpThirdPartyAppServer
call Edit_table('WebNmsDB','CnpThirdPartyAppServer','create index `FKF75D840E1798EF8` on CnpThirdPartyAppServer(MOID)');
...
SELECT 'STEP 217 : Altering DeviceAudit to increase TASKNAME size to solve blade lock error' AS 'MIGRATION PROCESS STATUS ... ';
alter table DeviceAudit modify column TASKNAME varchar(70);
