//$Id: SnmpNodeTable.java,v 1.6 2008/08/01 09:42:25 barathv Exp $
...
public class SnmpNodeTable implements JmxTableModelListener{
         // User code starts here
Vector moInDB;
String className = "SnmpNode";//No I18N
    String[] names = {"MoNameIndex","HostNetMask","SysDesc","SysName","SysOid","InheritingOid6","InheritingTableName6"};//No I18N
    Object[] values = null;
    Hashtable hash = new Hashtable();
    private CompositeData getCompositeData(SnmpNode  mo,String moName) throws Exception
    {
        values = new Object[names.length];
	values[0] = moName;
	values[1]=mo.getHostNetmask();
	values[2]=mo.getSysDescr();
	values[3]=mo.getSysName();
	values[4]=mo.getSysOID();
	if(values[1]==null)
	{
	     values[1]=" ";//No I18N
	}
	...
	values[5]=oid;
	values[6]=agentName.getChildTableName(oid);
	  try
	{
	...
	private SnmpNodeEntry entry = null;
	private String instrClassName = "com.adventnet.nms.jmxagent.SnmpNodeEntry"; //No I18N
	...
	/**
	 * Initialises the SnmpNodeTable
	 */
	public SnmpNodeTable()
	{
		table = new Hashtable();
	}
	public SnmpNodeTable(AdventNet_WebNMS_MIB_JMX agentRef)
	{
		agentName = agentRef;
		table = new Hashtable();
                // User code starts here
	 ...
	 * @return TabularData containing the rows corresponding the SnmpNodeTable
	 */
	public TabularData getSnmpNodeTable()
	{
		try{
			if(table != null)
	 ...
	 */
	public void setSnmpNodeTable(TabularData data) throws Exception
	{
		...
		for(Enumeration e = data.enumerate();e.hasMoreElements();)
		{
			...
			if(table != null)
				entry = (SnmpNodeEntry)Utilities.getEntryFromCompositeData(table, comp, indexNames, instrClassName);
			else if(vec != null)
				entry = (SnmpNodeEntry)Utilities.getEntryFromCompositeData(vec, comp, indexNames, instrClassName);
...
// User code starts here
    ...
    void initPSID()
    {
        psSelectMoID1 = agentName.rlAPI.getPreparedStatementID("SELECT MIN(ManagedObject."+RelationalUtil.getAlias("name") +") FROM ManagedObject , SnmpNode WHERE ManagedObject."+RelationalUtil.getAlias("moid")+ "= SnmpNode."+RelationalUtil.getAlias("moid"));//No I18N

        psSelectMoID2=agentName.rlAPI.getPreparedStatementID("SELECT MIN(ManagedObject."+RelationalUtil.getAlias("name") +") FROM ManagedObject , SnmpNode WHERE ManagedObject."+RelationalUtil.getAlias("moid")+ "= SnmpNode."+RelationalUtil.getAlias("moid")+ " AND ManagedObject."+RelationalUtil.getAlias("name")+ " >?");//No I18N

        psSelectCount = agentName.rlAPI.getPreparedStatementID("SELECT COUNT(" + RelationalUtil.getAlias("moid") + ") FROM SnmpNode");//No I18N
    }
    ...
    public CompositeData getSnmpNode(String[] name)
    {
        SnmpNode node=null;
        try
        {
            node=(SnmpNode)agentName.topoAPI.checkOut(name[0],0,false,true);
            if(node!=null)
            {
