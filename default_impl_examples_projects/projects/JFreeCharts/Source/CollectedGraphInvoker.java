/*
$Id: CollectedGraphInvoker.java,v 1.1.10.1 2012/06/19 05:38:54 wesley Exp $ 
*/
/** 
 *     <MENU-ITEM name = "Collected Statistic"
 *	         action_command ="INVOKE_CLASS:com.adventnet.nms.poll.graphs.CollectedGraphInvoker"
 *	         shortcut_key  = "P"
...
public class CollectedGraphInvoker implements CustomClassInterface
{
...
{ 
                    ...
                    if(protocol != null)
                    {
                        if(protocol.equalsIgnoreCase("SNMP")||protocol.equalsIgnoreCase("CORBA")||protocol.equalsIgnoreCase("KPI")|| protocolCheck!=-1)
                        {
                            processForDefault(selectedRow);
                        }
                        else
                        {
...
{
System.out.println("Cannot Plot the graph as the following message :"+e.getMessage()+" has been thrown");
}
    ...
    private void processForDefault(Properties selectedRow)
    {
        try 
        {
            DefaultCollectedGraphController cgc = new DefaultCollectedGraphController(selectedRow);
        } 
        catch (Exception ex) 
        {
        ex.printStackTrace();
