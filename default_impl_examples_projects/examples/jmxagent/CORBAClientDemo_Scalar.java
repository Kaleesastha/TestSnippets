/* $Id: CORBAClientDemo_Scalar.java,v 1.1.1.1 2006/11/16 15:39:58 gramkumar Exp $ */
 ...
 */
public class CORBAClientDemo_Scalar
{
	...
	CORBAClientDemo_Scalar corbaclientdemo = new CORBAClientDemo_Scalar();
	corbaclientdemo.connect();
	corbaclientdemo.setVal();
	corbaclientdemo.getVal();
    }
 	...
 	try
	{
	    ...
	    corbaClient.connect(hostName,Integer.parseInt(portNo),null);
	...
	{
	    System.out.println("Exception when connect  = " + e);
	}
    }
 	...
 	try
	{
		...
		{
		    corbaClient.invoke(objectName,opeInfo[i].getName(),values,types);
 	...
 	try
	{
		...
		{
		    Object result = corbaClient.invoke(objectName,opeInfo[i].getName(),new Object[]{},new String[]{});
	...
	{
	    System.out.println("Exception  when get = " + e);
	}
    }
}
