...
import com.adventnet.nms.fe.alert.*;
import com.adventnet.nms.db.*;
import com.adventnet.nms.fe.event.*;
import com.adventnet.nms.fe.topo.*;
import com.adventnet.nms.fe.config.*;
...
public class DBXmlUpdateTest  
{
	...
	private void startProcess(String WebNmsHome, String Database, String option)
	{
		...
		NmsFEUtil.setDatabaseName(PureServerUtils.getDatabaseName());        
		...
		else
		{
			try
			{
				//Use thisssss for a fresh test
				/*boolean remove = dbxmlutil.removeUserEntry("root");
				  executeTestCase001();
				  executeTestCase003();
				  executeTestCase004();
				  executeTestCase010();
	...
	private Connection getORACLEConnection()
	{
		try
		{
			...
			return DriverManager.getConnection("jdbc:oracle:thin:@nms4ms-test5:1521:nms4ms","rprabhu","rprabhu");
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	private void executeTestCase001()
	{
		...
		try
		{
			...
			//System.out.println("For testing: "+dbup.updateDB("root","Tree.xml"));
	...
	private void executeTestCase005()
	{
		...
		try
		{
			res5 = dbup.updateDB("guest","Tree.xml");
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	private void executeTestCase014()
	{
		...
		try
		{
			res14 = dbup.writeToXml("root","TreeForRoot.xml");
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	private void executeTestCase018()
	{
		...
		try
		{
			boolean res18 = dbup.writeToXml("root","Tree.xml");
		}
		catch(Exception e)
		{
			System.out.println("Exception is ....");
