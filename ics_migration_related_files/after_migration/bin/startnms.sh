...
while(true)
do
$JAVA_HOME/bin/java $JPROFILER_OPTIONS -cp $CLASS_PATH $DEBUGGING_OPTIONS -Djava.util.logging.manager=org.apache.juli.ClassLoaderLogManager -Djava.util.logging.config.file=$TOMCAT_HOME/conf/logging.properties   -DMOTO_JOURNAL_CM_LOCK=$MOTO_JOURNAL_CM_LOCK -Djavax.net.ssl.trustStore1=conf/Truststore.truststore -Dssl.port=$SSL_PORT -Dhttps.protocols1=TLSv1 -Dwebcontainer.port=$WEBCONTAINER_PORT -Djava.awt.headless=true -DNAT_NAME=$NAT_NAME_DEFINE -DNAT_PORT=$SSL_PORT -Dcatalina.home=$TOMCAT_HOME -Dcatalina.base=$TOMCAT_HOME -Dmysql.home=$MYSQL_HOME -Dwebserver.port=$WEBSERVER_PORT -Dresource_check="$WEBSERVER_PORT,$WEBCONTAINER_PORT,$TOMCAT_SHUTDOWNPORT" -Dwebserver.rootdir=$WEBSERVER_HOME -DV3ENGINEID=$COMMON_NAME -mx2000m  -Dcom.sun.management.jmxremote.port=16000 -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.snmp.interface=`hostname` -Dcom.sun.management.snmp.acl=false -Dcom.sun.management.snmp.port=16500  com.adventnet.nms.startnms.NmsMainBE NMS_BE_PORT 2000 BE_FE RMI ROOT_DIR $NMS_HOME $*
