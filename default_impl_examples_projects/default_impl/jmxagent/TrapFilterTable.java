//$Id: TrapFilterTable.java,v 1.3.6.1 2012/01/25 05:12:45 karen.r Exp $
...
/**
 * @Version :  6.0.0 Wed Feb 21 22:49:56 IST 2007
 ...
 * trapFilterTable group
 */


public class TrapFilterTable{
	...
	public void setAgentReference(AdventNet_WebNMS_MIB_JMX agentRef){
		...
		// User code starts here
                if(agentName.trapConfImpl.trapFilterTable != null)
                    table = agentName.trapConfImpl.trapFilterTable;
                // User code ends here
	}
	...
	private TrapFilterEntry entry = null;
	private String instrClassName = "com.adventnet.nms.jmxagent.TrapFilterEntry";//No I18N
	private String[] indexNames = new String[]{"TrapFilterIndex"};//No I18N
	...
	/**
	 * Initialises the TrapFilterTable
	 */
	public TrapFilterTable()
	{
		table = new Hashtable();
	}
	public TrapFilterTable(AdventNet_WebNMS_MIB_JMX agentRef)
	{
		agentName = agentRef;
		table = new Hashtable();
			// User code starts here
	 ...
	 * @return TabularData containing the rows corresponding the TrapFilterTable
	 */
	public TabularData getTrapFilterTable()
	{
	     // User code starts here
	    agentName.trapConfImpl.loadTrapDatas();
	    // User code ends here
		try{
			if(table != null)
	 ...
	 */
	public void setTrapFilterTable(TabularData data) throws Exception
	{
		...
		for(Enumeration e = data.enumerate();e.hasMoreElements();)
		{
			...
			if(table != null)
				entry = (TrapFilterEntry)Utilities.getEntryFromCompositeData(table, comp, indexNames, instrClassName);
			else if(vec != null)
				entry = (TrapFilterEntry)Utilities.getEntryFromCompositeData(vec, comp, indexNames, instrClassName);
			...
			if(entry != null)
				...
				for(Enumeration ce = comp.enumerate();ce.hasMoreElements();)
				{
				 ...
				 // User code starts here
                                try
                                    {
                                        if((entry.getSetFilter().intValue() == 1) && (!entry.getAlertFilterClassName().equals("")) && (!entry.getTrapFilterName().equals("")))//No I18N
                                            {
                                                agentName.trapConfImpl.setTrapDatas(entry);
                                            }
                                    }
                                catch(Exception excep)
                                    {
                                        //this will remove the entry even from the hash table.
                                        //fix done for sp3. bug reported by sathishs.
			...
			else if(comp.getOperationType().equals(CompositeData.DELETED))
			{
			      // User code starts here
                            if(true)
                                throw new AgentException("Cannot delete",CommonUtils.GENERR);//No I18N
                            // User code ends here
			...
			else if(comp.getOperationType().equals(CompositeData.MODIFIED))
			{
				...
				if (entry !=null)
				{
					for(Enumeration ce = comp.enumerate();ce.hasMoreElements();)
					{
						...
						try{
							// User code starts here
							if((key.equalsIgnoreCase("trapFilterName")) && (!entry.getTrapFilterName().equals("")))//No I18N
								throw new AgentException("Cannot modify filter name",CommonUtils.GENERR);//No I18N
							// User code ends here
					...
					catch(Exception xx)
					{
						...
						{
						 throw new AgentException("Invalid entry :"+exce.getMessage(),CommonUtils.GENERR);//No I18N	
						}
						throw new AgentException("invalid entry",CommonUtils.GENERR);//No I18N
					}
				}
				// User code ends here
			}
