...
public class snmpget implements RunProcessInterface {
...
//-----------------------Connecting DB
			try
                        {
                       		target.setV3DatabaseFlag(true);
				DBParamsParser parser = DBParamsParser.getInstance();
				String URL = parser.getURL();
				String userName = parser.getUserName();
				String password = parser.getPassword();
				String driver = parser.getDriverName();
				//target.initJdbcParams("org.postgresql.Driver","jdbc:postgresql://localhost/final","postgres","postgres");
				target.initJdbcParams(driver, URL, userName, password);
...
//target.setContextName("auth");
//System.out.println(target.getContextName());
