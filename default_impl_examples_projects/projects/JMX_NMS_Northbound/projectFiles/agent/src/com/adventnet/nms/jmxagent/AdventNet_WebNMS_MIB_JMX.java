 //$Id: AdventNet_WebNMS_MIB_JMX.java,v 1.19.4.1.2.1 2014/06/13 11:48:04 swaminath Exp $
...
import com.adventnet.agent.tl1.TL1AgentResponseInfo;
 ...
 boolean newEnterpriseOID = true;
	...
	*/
	...
	public AdventNet_WebNMS_MIB_JMX(String[] args)
	{
         ...
         */
        corbaAdaptor= getArgString("CORBA").equalsIgnoreCase("true")? true:false;//no i18n
        rmiAdaptor= getArgString("RMI").equalsIgnoreCase("true")? true:false;//no i18n
        httpAdaptor= getArgString("HTTP").equalsIgnoreCase("true")? true:false;//no i18n
        htmlAdaptor= getArgString("HTML").equalsIgnoreCase("true")? true:false;//no i18n
        snmpAdaptor= getArgString("SNMP").equalsIgnoreCase("true")? true:false;//no i18n
        tl1Adaptor= getArgString("TL1").equalsIgnoreCase("true")? true:false;//no i18n
        topoObsvr = getArgString("TopoObserver").equalsIgnoreCase("true")? true:false;//no i18n
        faultObsvr = getArgString("FaultObserver").equalsIgnoreCase("true")? true:false;//no i18n
        doPersist = getArgString("Persist").equalsIgnoreCase("true")? true:false;//no i18n
        const_EngineID = getArgString("Constant_V3EngineID").equalsIgnoreCase("true")? true:false;//no i18n
        authentication = getArgString("Authentication").equalsIgnoreCase("false")? false:true;//no i18n
...
// User code starts here
    ...
    {
    	int retVal=-1;
    	String value= argProps.getProperty(text.toLowerCase());
    	try
    	{
    		retVal=Integer.parseInt(value);
    	}
    	catch(NumberFormatException nfe){}
    	//if $NMS_REG_PORT is given as value, we will reach here and -1 will be returned,
    	//if the value is null or non integer, -1 will be returned.
    	return retVal;
    }

    String getArgString(String text)
    {
    	return argProps.getProperty(text.toLowerCase(),""); //no i18n
    }
    /*
...
/*
   ...
   private int seqNumIncrement()
    ...
    public void begin() throws UserTransactionException
    {
	    try{
		    rlAPI.getTransactionAPI().begin();
    ...
    public void commit() throws UserTransactionException
    {
	    try {
		    rlAPI.getTransactionAPI().commit();
	    }
    ...
    {
	    rlAPI.getTransactionAPI().rollback(str);
    }

    private Properties argProps = new Properties();
    ...
    private void readCommandLineArgs(String args[])
    {
	    ...
	    if(argc %2 != 0 )
	    {
    	 //this is to avoid AIOB exception.. lets omit the last args under the assumption, the  user had missed the value for a key.. 
     	argc=argc-1;
     }
     for(int i = 0 ; i < argc ; i = i + 2)
     {
     	argProps.setProperty(args[i].toLowerCase(), args[i+1].toLowerCase());
     }
 }
// User code ends here
}
