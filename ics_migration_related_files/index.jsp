<%-- $Id: index.jsp,v 1.31.2.3.4.4.4.3 2006/09/27 11:02:06 johnpaul Exp $ --%>
...
if(ht.containsKey("fromAppClient")) 
{
	//added for SP7
	if(htmlui_frames.equalsIgnoreCase("true"))
	{
	    String appletUrl=(String)request.getParameter("fromAppClient");
	    if(appletUrl.indexOf("../")!=-1)
		appletUrl=appletUrl.substring(appletUrl.indexOf("../")+3);
		else if(appletUrl.indexOf("/jsp")!=-1)
		appletUrl=appletUrl.substring(appletUrl.indexOf("/jsp")+1);
		Element root=null;
		try 
		{   
			Connection  con = NmsUtil.relapi.getConnection();
			TreeAPI tree= new TreeAPI(con);
			root = tree.getNodeFromDB(userName,null);
			if(getNodeToOpen(appletUrl,root)!=null)
			nodesToOpen=getNodeToOpen(appletUrl,root);
		}
		catch(Exception e) {
			NmsLogMgr.MISCERR.fail(NmsUtil.GetString("Exception in getting Node from TreeAPI")+e.getMessage(), e);
		}
		String reqString = "../jsp/ShowImage.jsp?imageName=../images/screen.png";
		if(getNodeToOpen(appletUrl,root)==null)
		{
			nodesToOpen = "Maps";
			reqString = "/"+appletUrl;
		...
		{
			Element node = searchForElement(nodesToOpen,root,out);
			if (node == null)
			{
				node = getLeafNode(root);
			}
			reqString = getReqString(node);
			application.setAttribute("elementFor"+userName,root);
		}
		%>
			<SCRIPT>
			window.focus();
			...
			<FRAMESET COLS="220,*" >
			<FRAMESET ROWS="*,100" >
			<FRAME NAME="tree" SRC="/jsp/NmsTree.jsp?fromIndexPage=<%=URLEncoder.encode(nodesToOpen)%>&completeRefresh=" SCROLLING="AUTO" MARGINWIDTH="1" MARGINHEIGHT="6">
			<FRAME NAME="status" SRC="/jsp/Status.jsp" MARGINWIDTH="0" MARGINHEIGHT="5" TOPMARGIN="5" SCROLLING="AUTO">
			</FRAMESET>
			<FRAME NAME="center" SRC="<%=reqString%>" SCROLLING="AUTO" >
			</FRAMESET>
			<%
	}
	else
	{	
		%>
			...
			<FRAME NAME="usersFile" SRC='<%= (String)ht.get("fromAppClient")%>?<%=queryStr%>' NORESIZE SCROLLING="AUTO">
			<FRAME NAME="dummy" SRC="../html/dummy.html" NORESIZE SCROLLING="NO">
			</FRAMESET>
			<%
	}
}
else if(ht.containsKey("htmlui")) 
{
	Element root=null;
	//added for SP7 
	String htmlUrl=(String)ht.get("uri");
	try 
	{   
		Connection  con = NmsUtil.relapi.getConnection();
		TreeAPI tree= new TreeAPI(con);
	...
	if(root!=null) 
	{
		...
		if(htmlUrl!=null && htmlui_frames.equalsIgnoreCase("true"))
		{
			//added for SP7 From url passed , node to be opened will be found .If no nodes matches 
			// then, tree will be opened to first level.
			String uriPassed=htmlUrl.substring(htmlUrl.indexOf("/")+1);
			nodesToOpen=getNodeToOpen(uriPassed,root);
			if(nodesToOpen==null)
				nodesToOpen = "Maps";
			if(!htmlUrl.equals("/jsp/ShowImage.jsp") && !htmlUrl.equals("/jsp/NmsTree.jsp") && !htmlUrl.equals("/jsp/Status.jsp") && !htmlUrl.equals("/jsp/FrameSet.jsp"))
			reqString=htmlUrl;
