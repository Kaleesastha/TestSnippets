 //$Id: NetworkDiscoveryTable.java,v 1.2 2007/04/30 17:39:31 tinku Exp $
 ...
 * networkDiscoveryTable group
 */


public class NetworkDiscoveryTable
{
	...
	if(agentName.topoConfImpl.networkDiscoveryTable != null)
	    table = agentName.topoConfImpl.networkDiscoveryTable;
	// User code ends here
    }
	...
	private NetworkDiscoveryEntry entry = null;
	private String instrClassName = "com.adventnet.nms.jmxagent.NetworkDiscoveryEntry";//No I18N
	private String[] indexNames = new String[]{"NetworkDiscoveryIndex"};//No I18N
	...
	/**
	 * Initialises the NetworkDiscoveryTable
	 */
	public NetworkDiscoveryTable()
	{
		table = new Hashtable();
	}
	public NetworkDiscoveryTable(AdventNet_WebNMS_MIB_JMX agentRef)
	{
		agentName = agentRef;
		table = new Hashtable();
	}
	 ...
	 * @return TabularData containing the rows corresponding the NetworkDiscoveryTable
	 */
	public TabularData getNetworkDiscoveryTable()
	{
		// User code starts here
	agentName.topoConfImpl.loadSeedDatas();
	// User code ends here
		try{
			if(table != null)
	 ...
	 */
	public void setNetworkDiscoveryTable(TabularData data) throws Exception
	{
		...
		for(Enumeration e = data.enumerate();e.hasMoreElements();)
		{
			...
			if(table != null)
				entry = (NetworkDiscoveryEntry)Utilities.getEntryFromCompositeData(table, comp, indexNames, instrClassName);
			else if(vec != null)
				entry = (NetworkDiscoveryEntry)Utilities.getEntryFromCompositeData(vec, comp, indexNames, instrClassName);
			...
			if(entry != null)
				...
				entry = new NetworkDiscoveryEntry();
			...
			else if(comp.getOperationType().equals(CompositeData.MODIFIED))
			{
			 ...
			 // User code starts here
	    if(!agentName.topoConfImpl.setSeedData(3))
