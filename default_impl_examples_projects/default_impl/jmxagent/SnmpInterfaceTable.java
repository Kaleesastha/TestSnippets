//$Id: SnmpInterfaceTable.java,v 1.6 2008/08/01 09:42:05 barathv Exp $
 ...
 * snmpInterfaceTable group
 */


public class SnmpInterfaceTable implements JmxTableModelListener{
          // User code starts here
Vector moInDB;
String className = "SnmpInterface";//No I18N
    String[] names = {"MoNameIndex","HostnetMask","IfIndex","PhysMedia", "PhysAddress","IfSpeed","IfDesc","SysOID","InheritingOid7","InheritingTableName7"};//No I18N
    Object[] values = null;
    Hashtable hash = new Hashtable();
     private CompositeData getCompositeData(SnmpInterface  mo,String moName) throws Exception
    {
        values = new Object[names.length];
	values[0] = moName;
	values[1]=mo.getHostNetmask();
	if(values[1]==null)
	{
	     values[1]=" ";//No I18N
	}
	values[2]=new Integer(mo.getIfIndex());
	values[3]=mo.getPhysMedia();
	if(values[3]==null)
	{
	     values[3]=" ";//No 18N
	}
	values[4]=mo.getPhysAddr();
	if(values[4]==null)
	{
	     values[4]=" ";//No 18N
	}
	values[5]=new Integer(mo.getIfSpeed());
	values[6]=mo.getIfDescr();
	if(values[6]==null)
	{
	     values[6]=" ";//No 18N
	}
	values[7]=mo.getSysOID();
	if(values[7]==null)
	{
	     values[7]=" ";//No 18N
	}
	...
	values[8]=oid;
	values[9]=agentName.getChildTableName(oid);
	  try
	{
	...
	private SnmpInterfaceEntry entry = null;
	private String instrClassName = "com.adventnet.nms.jmxagent.SnmpInterfaceEntry";//No I18N
	...
	/**
	 * Initialises the SnmpInterfaceTable
	 */
	public SnmpInterfaceTable()
	{
		table = new Hashtable();
	}
	public SnmpInterfaceTable(AdventNet_WebNMS_MIB_JMX agentRef)
	{
		agentName = agentRef;
		table = new Hashtable();
                // User code starts here
	 ...
	 * @return TabularData containing the rows corresponding the SnmpInterfaceTable
	 */
	public TabularData getSnmpInterfaceTable()
	{
		try{
			if(table != null)
	 ...
	 */
	public void setSnmpInterfaceTable(TabularData data) throws Exception
	{
		...
		for(Enumeration e = data.enumerate();e.hasMoreElements();)
		{
			...
			if(table != null)
				entry = (SnmpInterfaceEntry)Utilities.getEntryFromCompositeData(table, comp, indexNames, instrClassName);
			else if(vec != null)
				entry = (SnmpInterfaceEntry)Utilities.getEntryFromCompositeData(vec, comp, indexNames, instrClassName);
...
// User code starts here
    ...
    void initPSID()
    {
        psSelectMoID1 = agentName.rlAPI.getPreparedStatementID("SELECT MIN(ManagedObject."+RelationalUtil.getAlias("name") +") FROM ManagedObject , SnmpInterface WHERE ManagedObject."+RelationalUtil.getAlias("moid")+ "= SnmpInterface."+RelationalUtil.getAlias("moid"));//No I18N

        psSelectMoID2=agentName.rlAPI.getPreparedStatementID("SELECT MIN(ManagedObject."+RelationalUtil.getAlias("name") +") FROM ManagedObject , SnmpInterface WHERE ManagedObject."+RelationalUtil.getAlias("moid")+ "= SnmpInterface."+RelationalUtil.getAlias("moid")+ " AND ManagedObject."+RelationalUtil.getAlias("name")+ " >?");//No I18N

        psSelectCount = agentName.rlAPI.getPreparedStatementID("SELECT COUNT(" + RelationalUtil.getAlias("moid") + ") FROM SnmpInterface");//No I18N
    }
    ...
    public CompositeData getSnmpIpAddress(String[] name)
    {
        SnmpInterface ip=null;
        try
        {
            ip=(SnmpInterface)agentName.topoAPI.checkOut(name[0],0,false,true);
            if(ip!=null)
            {
