#Procedure to check if a table exists before altering it
DELIMITER $$
DROP PROCEDURE IF EXISTS `Edit_table` $$
CREATE PROCEDURE `Edit_table` (db_name varchar(20),tbl_name varchar(20), my_query varchar(200))
DETERMINISTIC
BEGIN
DECLARE var_table_count INT;

select count(*) INTO @var_table_count from information_schema.TABLES where  TABLE_NAME=tbl_name and TABLE_SCHEMA=db_name;
IF (@var_table_count > 0) THEN
SET @my_query = my_query;
PREPARE actual_query FROM @my_query;
EXECUTE actual_query;
ELSE
select 'Table Not Found' AS 'QUERY RESULT';
END IF;
...
SELECT 'Dropping foreign keys' AS 'MIGRATION PROCESS STATUS ... ';
...
######## 4.7 ICS CUSTOM FOREIGN KEYS - NO NEED TO DROP
...
######## 4.7 ICS CUSTOM FOREIGN KEYS ENDS ###############
...
SELECT 'Changing PolledData' AS 'MIGRATION PROCESS STATUS ... ';
...
alter table ALERTUSERPROPS add column OWNERNAME varchar(25) NOT NULL default 'NULL' after NAME;
...
alter table ANNOTATION add column OWNERNAME varchar(25) after MODTIME;
...
alter table MapDB change column ANCHORED ANCHORED varchar(10) after AUTOPLACEMENT;
...
alter table MapSymbol change column OBJNAME OBJNAME varchar(100) after NAME;
...
alter table MapContainer add column CONTAINMENT_STR varchar(10) after CONTAINMENT;
...
alter table MapContainer add column OBJTYPE INTEGER after CONTAINMENT;
...
alter table MapLink add column X varchar(25) after NY;
alter table MapLink add column Y varchar(25) after X;
...
alter table MapLink change column SOURCE SOURCE varchar(100) after NAME;
alter table MapLink change column DEST DEST varchar(100) after SOURCE;
...
alter table MapGroup add column OWNERNAME varchar(25) not NULL after MAPNAME;
...
alter table ManagedObject add column OWNERNAME varchar(25) not NULL default 'NULL' after WEBNMS;
...
alter table ManagedObject change column ISGROUP ISGROUP varchar(10) after ISCONTAINER;
update ManagedObject set ISGROUP='true' where ISGROUP like '1';
update ManagedObject set ISGROUP='false' where ISGROUP like '0';

alter table ManagedObject change column MANAGED MANAGED varchar(10) after TYPE;
...
alter table TOPOUSERPROPS add column OWNERNAME varchar(25) NOT NULL default 'NULL' after NAME;
...
alter table EMHEvent  add column OWNERNAME varchar(25) after ID;
alter table EMHInfoEvent add column OWNERNAME varchar(25) after ID;
alter table EMHAlert add column OWNERNAME varchar(25) after ENTITY;
alter table ObjectCreationDeletionEvent add column OWNERNAME varchar(25) after ID;
alter table StateChangeEvent add column OWNERNAME varchar(25) after ID;
alter table UserDefinedEvent add column OWNERNAME varchar(25) after ID;
alter table AttributeValueChangeEvent add column OWNERNAME varchar(25) after ID;
alter table AvailabilityEvent add column OWNERNAME varchar(25) after ID;
alter table RelationshipChangeEvent add column OWNERNAME varchar(25) after ID;
alter table CNEOMIManagementEvent add column OWNERNAME varchar(25) after ID;
...
SELECT 'adding PRIMARY KEYS 2' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 3' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 4' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 5' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 6' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 7' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 8' AS 'MIGRATION PROCESS STATUS ... ';
alter table CnpShmmBMC add PRIMARY KEY(NAME,OWNERNAME);
alter table CnpEmsSensor add PRIMARY KEY(NAME,OWNERNAME);
call Edit_table('WebNmsDB','CnpThirdPartyAppServer','alter table CnpThirdPartyAppServer add PRIMARY KEY(NAME,OWNERNAME)');
...
SELECT 'adding PRIMARY KEYS 9' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 10' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 11' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 12' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 13' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 14' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS 15' AS 'MIGRATION PROCESS STATUS ... ';
...
SELECT 'adding PRIMARY KEYS ENDS' AS 'MIGRATION PROCESS STATUS ... ';
################### PRIMARY KEYS ENDS #############
drop table MAPPEDPROPERTIES;
